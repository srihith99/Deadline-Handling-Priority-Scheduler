...
void edf :: execute_window(string& running_process,int next_time)
{
...
// This functions checks if the process p's period predecessor has missed it's deadline and then proceeds to insert p in ready_processes 
void edf :: insert_into_ready(process p)
{
...
string edf :: highest_priority_from_ready()
{
	// In edf , lower the deadline higher the priority
	string best_process = "None";
	int least_deadline = INT_MAX;
	for(map<string,process> :: iterator it = ready_hashmap.begin();it != ready_hashmap.end();it++)
	{
		string current_pid = it->first;
		int current_deadline = all_processes_hashmap[current_pid].period*(it->second.iteration);
		if(current_deadline < least_deadline)
		{
			least_deadline = current_deadline;
...
{	
	...
	//Invoke the edf constructor
	edf simulation(input_processes);
	//Run the simulation
	simulation.start();
